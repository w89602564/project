...
if (system.args.length !== 2) {
  ...
  service = server.listen(port, {
    ...
    page.settings.allowMedia = false;
    if (fetch.headers) {
    ...
    page.onInitialized = function() {
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        // page_loaded = true;
        // end_time = Date.now();
        console.log('==============onInitializedonInitializedonInitializedonInitialized====================');
    ...
    page.onResourceRequested = function(requestData, request) {
      ...
      end_time = null;
      if (page_loaded==false){end_time = null;}
      else{
        // request.abort();
        // console.log("request abort: #"+requestData.id+" ["+requestData.method+"]"+requestData.url);
      }
    };
    page.onResourceReceived = function(response) {
      // console.log("Request finished: #"+response.id+" ["+response.status+"]"+response.url);
      if (response.contentType == 'application/json') {
        visited_json.push(JSON.stringify(response.body, undefined, 4));
      }
    ...
    page.onResourceError = page.onResourceTimeout=function(response) {
      console.debug("Request error: #"+response.id+" ["+response.errorCode+"="+response.errorString+"]"+response.url);
